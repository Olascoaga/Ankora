"""Create or verify the six accepted LIT-PCBA benchmark receptors."""
...
def _arguments() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Create six final receptor/PDBQT derivatives from the accepted "
            "pre-result plans, or verify their path-free manifest."
        )
    )
    ...
    parser.add_argument("--tautomer-verification", type=Path)
    parser.add_argument("--protonation-decisions", type=Path)
...
def _required(value: Path | None, option: str) -> Path:
    if value is None:
        raise ValueError(f"{option} is required when creating final receptors.")
    return value
    ...
    try:
        if args.check:
            ...
            manifest = run_screening_benchmark_final_receptors(
                ...
                tautomer_manifest_path=_required(
                    args.tautomer_verification, "--tautomer-verification"
                ),
                protonation_decision_review_path=_required(
                    args.protonation_decisions, "--protonation-decisions"
                ),
        ...
        print(f"Final receptor run failed: {error}", file=sys.stderr)
        return 1
    census = manifest["receptor_census"]
    print(
        f"{manifest['protocol_id']}: {census['completed']}/"
        f"{census['requested']} final receptors completed; "
        f"{census['pdbqt_created']} PDBQT created; no docking executed; "
